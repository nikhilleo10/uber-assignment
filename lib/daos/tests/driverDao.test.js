...
describe('driverDao', () => {
  describe('getNearbyDrivers', () => {
    ...
    it('should return list of nearby drivers to a given point', async () => {
      ...
      await resetAndMockDB((db) => {
        ...
        for (let i = 0; i < limit; i++) {
            arr.push(db.models.vehicle.build())
        }
        db.models.vehicle.$queueResult(arr);
        findAllSpy = jest.spyOn(db.models.vehicle, 'findAll');
      });
    ...
    it('should return empty array', async () => {
      let findAllSpy;
      await resetAndMockDB((db) => {
        db.models.vehicle.findAll = () => new Promise((resolve) => resolve([]));
        findAllSpy = jest.spyOn(db.models.vehicle, 'findAll');
      });
